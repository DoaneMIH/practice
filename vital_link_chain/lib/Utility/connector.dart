...
class Connector {
  ...
  static Future<DeployedContract> getContractHealthSystem() async {
    ...
    String contractAddress =
        "0x04713d4A2C0D6e6257B9eC054749e314E598E290"; // Replace with your deployed contract address
  ...
  static Future<DeployedContract> getContractMedicalCertificate() async {
    ...
    String abi = jsonEncode(jsonAbi['abi']);
    String contractAddress =
        "0x9e4db9f914cc853a33e1d8c22f57b85Ab919fCc0"; // Replace with your deployed contract address
    ...
    static Future<DeployedContract> getContractMedicalExtension() async {
    String abiFile = await rootBundle.loadString(
      "build/contracts/HealthGet.json",
    );
    Map<String, dynamic> jsonAbi = jsonDecode(abiFile);
    String abi = jsonEncode(jsonAbi['abi']);
    String contractAddress =
        "0xF31E6E61Eb43543960534755F36ac19C200A3fEd"; // Replace with your deployed contract address

    final contract = DeployedContract(
      ContractAbi.fromJson(abi, "HealthGet"),
      EthereumAddress.fromHex(contractAddress),
    );
...
static Future<bool> revokeAccess(String patientPrivateKey, String doctorAddress) async {
  ...
  try {
    ...
    final name = await getName(patientAddressHex);
    ...
    if (receipt != null && receipt.status == true) {
      Fluttertoast.showToast(
        msg: "Access request sent to patient $name",
...
// static Future<List<Map<String, String>>> getPresc(EthereumAddress address) async {
//   try {
//     final contract = await getContractHealthSystem();
//     final result = await ethClient.call(
//       contract: contract,
...
//     // result[0] should be List<dynamic> of string prescriptions
//     final prescriptions = (result[0] as List<dynamic>).cast<String>();

//     // Extract actual prescription and txHash
//     final processed = prescriptions.map((entry) {
//       final parts = entry.split('#');
//       final prescription = parts[0];
//       final txHash = parts.length > 1 ? parts[1] : "Not available";
//       return {
...
// static Future<String> setPrescription(
//   String content,
//   String patientAddress,
...
//   // Create a temporary dummy transaction to get the txHash *before* sending
...
//   // Append txHash to content as metadata and send it again
//   final contentWithHash = "$content#$txHash";

//   // Send updated prescription with hash
//   await ethClient.sendTransaction(
//     credentials,
//     Transaction.callContract(
//       contract: contract,
...
static Future<String> issueCertificate(String content, String patientAddress, String doctorPrivateKey) async {
  try {
    print("issueCertificate called with content: $content, patientAddress: $patientAddress");
    final credentials = EthPrivateKey.fromHex(doctorPrivateKey);
    final contract = await getContractMedicalCertificate();
    ...
    print("Certificate issued successfully! txHash: $txHash");
    // Fluttertoast.showToast(msg: "Certificate issued successfully!");
    return txHash;
  } catch (e, stack) {
    print("Error in issueCertificate: $e");
...
static Future<void> registerDoctorOnMedicalCertificate(String doctorPrivateKey, String doctorName) async {
  try {
    ...
    print("[CORRECT] Doctor registered on MedicalCertificate!");
...
static Future<dynamic> getPrescriptionByTxHash(String txHash) async {
  try {
    final receipt = await ethClient.getTransactionReceipt(txHash);
    if (receipt != null && receipt.status == true) {
      // Optionally, decode logs or fetch prescription details
      return receipt;
    }
    return null;
  } catch (e) {
    print("Error verifying prescription by txHash: $e");
    return null;
  }
}


static Future<void> registerDoctorOnDoctorforAll(String doctorPrivateKey, String doctorName) async {
  try {
    final credentials = EthPrivateKey.fromHex(doctorPrivateKey);
    final contract = await getContractMedicalExtension();
    final function = contract.function('addDoctor');
    // print("[DEBUG] Registering doctor on MedicalCertificate: $doctorName");
    await ethClient.sendTransaction(
      credentials,
    ...
    print("[CORRECT] Doctor registered!");
    // Fluttertoast.showToast(msg: "Doctor registered on MedicalCertificate!");
  } catch (e) {
    print("[ERROR] Failed to register doctor: $e");
    Fluttertoast.showToast(msg: "Error registering doctor: $e");
...
static Future<List<Map<String, dynamic>>> getAllDoctors() async {
  final contract = await getContractMedicalExtension();
  final getAllDoctorsFunction = contract.function('getAllDoctors');
  final result = await ethClient.call(
    contract: contract,
    function: getAllDoctorsFunction,
    params: [],
  );
  if (result.isEmpty || result[0] == null || result[1] == null) {
    print("No doctors found in contract.");
    return [];
  }
  final List addresses = result[0] as List;
  final List names = result[1] as List;
  List<Map<String, dynamic>> doctors = [];
  for (int i = 0; i < addresses.length; i++) {
    doctors.add({
      "address": addresses[i].toString(),
...
static Future<void> registerPatientforAll(String privateKey, String patientName) async {
  try {
    final credentials = EthPrivateKey.fromHex(privateKey);
    final contract = await getContractMedicalExtension();
    final function = contract.function('addPatient');
    ...
    print("[CORRECT] Patient registered!");
    // Fluttertoast.showToast(msg: "Patient registered on MedicalExtension!");
  } catch (e) {
    print("[ERROR] Failed to register patient: $e");
    Fluttertoast.showToast(msg: "Error registering patient: $e");
  }
}
...
static Future<List<Map<String, dynamic>>> getAllPatients() async {
  final contract = await getContractMedicalExtension();
  final getAllDoctorsFunction = contract.function('getAllPatient');
  final result = await ethClient.call(
    contract: contract,
    function: getAllDoctorsFunction,
    params: [],
  ...
  if (result.isEmpty || result[0] == null || result[1] == null) {
    print("No patiet found in contract.");
    return [];
  }
  final List addresses = result[0] as List;
  final List names = result[1] as List;
...
static Future<bool> isUserExists(String address) async {
  try {
    final contract = await getContractHealthSystem();
    final function = contract.function('isExists');
    ...
    // result is a List<dynamic> with one bool value
