...
import 'package:vital_link/Pages/view_prescription.dart';
...
void setAuthorization() async {
  try {
    print("Doctor Address: ${doctorAddress.text}"); // Debug print
    if (doctorAddress.text.isEmpty || doctorAddress.text.length < 40) {
      Fluttertoast.showToast(msg: "Invalid Address");
      return;
    }

    // Validate the Ethereum address
    EthereumAddress doctorAddr = EthereumAddress.fromHex(doctorAddress.text);

    // Check if the doctor exists
    bool doctorExists = await Connector.isDoctor(doctorAddr);
    if (!doctorExists) {
      Fluttertoast.showToast(msg: "Doctor does not exist!");
      return;
    }

    // Call the addAuthorization method
    ...
    if (!isAuthorized) {
      ...
      Fluttertoast.showToast(
          msg: "Doctor is now authorized to give you prescriptions.");
      doctorAddress.clear();
    }
  } catch (e) {
    Fluttertoast.showToast(msg: "Error: ${e.toString()}");
    print("Error in setAuthorization: $e");
  }
}
...
//   _showPickerRevokeAuthorization() {
//   showModalBottomSheet(
//       isScrollControlled: true,
//       shape: const RoundedRectangleBorder(
//           borderRadius: BorderRadius.vertical(top: Radius.circular(25.0))),
...
//                   InkWell(
//                       onTap: () async {
//                         revokeAuthorization(doctorAddress.text);
//                         Navigator.pop(context);
//                         doctorAddress.clear();
//                       },
//                       child: const SizedBox(
...
// }
  ...
  Widget build(BuildContext context) {
    ...
    // InkWell(
    //   onTap: () => _showPickerRevokeAuthorization(),
    //   child: Container(
    //     width: (MediaQuery.of(context).size.width / 1.5),
    //     height: 50,
    //     decoration: BoxDecoration(
